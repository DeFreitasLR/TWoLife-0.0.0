...
* `plot_landscape()` - Display landscape habitat quality with custom coloring
* `population_size()` - Extract and analyze population trajectories
* `snapshot_at_time()` - Reconstruct spatial distribution at specific time points
* `check_habitat_match()` - Visualize individual trait-habitat matching
* `habitat_mismatch()` - Quantify fitness-based genotype-habitat mismatch
...
* Complete help documentation for all 8 exported functions
...
* 7 C++ source files with modular design
* 14 documentation files (.Rd format)
