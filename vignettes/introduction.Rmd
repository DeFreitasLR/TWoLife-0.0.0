...
-   **Continuous landscapes**: Environmental values vary along
    ...
    30°C, ideally rescaled to 0-1), soil moisture (0% to 100%), or
    resource availability. In continuous landscapes, no locations are
    inherently unsuitable. Rather, suitability emerges dynamically from
    the interaction between individual phenotypes and local
    environmental values through Gaussian matching (explained in the
    Fitness section below). This means that a cell with value 0.3 is not
    intrinsically "better" or "worse" than a cell with value 0.7 —
...
exhibits scale-invariant patterns.
...
Rather than acting directly as a survival probability, fitness in
TWoLife is used primarily to modulate mortality rate. Mortality is
bounded between a baseline rate experienced under optimal conditions
(basal mortality) and a maximum rate associated with highly unsuitable
environments, defined as the basal mortality multiplied by the matrix
mortality multiplier. Fitness determines where, within this range, an
individual’s mortality falls:

$$
\text{mortality rate}=\text{maximum mortality}-\text{fitness}\left(\text{maximum mortality}-\text{basal mortality}\right)
$$

Under this formulation, individuals experience minimum mortality when
local environmental conditions closely match their phenotype, and
mortality increases smoothly as conditions deviate from the individual’s
optimum. This implements stabilizing selection through
environment-dependent survival while keeping mortality rates
biologically interpretable and bounded.
...
their ecological niches and respond differently to their environment.
...
-   **`genotype_sd`** (Niche width/tolerance): The breadth of
    ...
    across diverse conditions albeit with lower peak fitness.
...
environmental gradients (demonstrated in Section 6).
...
ensures all random processes produce identical results when re-run with
the same seed value. Using identical master_seed with identical
parameters produces exactly identical results, while using different
seeds with identical parameters produces independent iterations. This
distinction is analogous to experimental trials: the processes are
identical but random outcomes differ, allowing quantification of outcome
...
In several small (right), approximately 20% of cells are distributed
across multiple isolated patches, creating numerous edges and requiring
dispersal between fragments. These spatial differences have demographic
consequences: single large patches support reproduction within
continuous habitat, while several small patches require successful
inter-patch dispersal for population persistence. All three will be
initialized with 100 individuals to test how spatial arrangement affects
population outcomes when habitat amount differs (pristine versus
fragmented) and when habitat amount is held constant but configuration
varies (single large versus several small):
...
**genetic_params**: Omitted in this example, which applies default
values appropriate for binary landscapes (see Section 4.1 for details).
These defaults create genetically uniform populations suitable for
exploring landscape structure effects without genetic complications.
...
produce independent realizations of the same scenario.
...
This section presents a worked example illustrating how TWoLife models
populations in heterogeneous environments when individuals differ in
traits leading to resource usage. Rather than using discrete
habitat–matrix classifications or genetically uniform populations, this
analysis focuses on phenotype–environment matching along environmental
gradients.

The goal of this example is to isolate the contribution of specific
biological mechanisms to population persistence. We do this by
implementing five simulation scenarios that differ only in genetic and
behavioral parameters, while holding all other conditions constant
(landscape structure, initial population size, simulation duration, and
random seeds).

The scenarios progressively add mechanisms in the following order:
genetic diversity in optimal trait values, tolerance (niche width),
evolutionary processes (mutation and plasticity), and habitat selection
behavior. This progression allows direct comparison of which mechanisms
are necessary for persistence and which provide additional benefits once
persistence is possible.

The sections below implement each scenario in turn and interpret their
outcomes.
...
The progressive design builds each scenario on the previous one by
adding a single mechanism. This approach allows population changes to be
attributed to specific mechanisms and enables testing of how mechanisms
combine: additively, where each additional mechanism contributes
independently, or synergistically, where combined effects exceed the sum
of individual contributions. It also makes it possible to identify
patterns of diminishing returns, where later additions provide smaller
benefits, or accelerating returns, where later additions yield larger
benefits, potentially through synergistic interactions.
...
matches that do not exist in the continuous landscape (range:
`r sprintf("%.2f", min(continuous_landscape))` to
`r sprintf("%.2f", max(continuous_landscape))`). Any mismatch produces
maximum mortality rates, demonstrating that genetic uniformity combined
with specialist strategies is incompatible with persistence in
heterogeneous continuous environments.
...
has `genotype_mean = 0.5` and a cell has `environment = 0.5`, slight
mismatches due to movement produce effectively zero fitness when
...
sim_s3 <- twolife_simulation(
  ...
  genetic_params = list(
    ...
    genotype_sds = rep(0.2, 40)            # Add tolerance: key difference from S2
  ),
  simulation_params = list(max_events = 100),
  master_seed = 47
)
...
randomly (sampling_points=0) and genetic composition cannot change over
...
from Scenario 3). Evolution provides reduced performance beyond static
genetic diversity and tolerance. Mutation allows the population to
explore genetic space beyond the initial genotype distribution
(0.3-0.7), potentially adapting to local environmental conditions if
selection favors particular genotypes. Plasticity allows phenotypes to
deviate from genotypes, which can be beneficial if it increases variance
in trait-environment matching, though it can also be detrimental if it
moves phenotypes away from local optima. The net effect depends on the
balance between these processes over the simulation duration.
...
sim_s5 <- twolife_simulation(
  ...
  genetic_params = list(
    ...
    sampling_points = rep(1000, 40)  # Enable habitat selection
  ),
  simulation_params = list(max_events = 100),
  master_seed = 50
)
...
selection mitigated the consequences of locally maladapted mutations by
allowing offspring with unfit local traits to actively seek favorable
...
or genotype values (right). Both panels share the same color scale: low
values are cool (blue/green), intermediate values are yellowish, and
high values are warm (yellow/red). Effective matching is indicated when
survivor dots visually “blend” with the underlying landscape gradient.
...
genotype toward better local adaptation. Over the short simulation (250
...
(`sampling_points=100`) and preferentially settle where their phenotype
...
1.  **Same master_seed**: The `master_seed` parameter controls all
    stochasticity
2.  **Same TWoLife version**: Algorithm implementations may change
    between versions
3.  **Same R version**: Underlying random number generators may differ
    across R versions

The `master_seed` ensures that identical parameter sets produce
identical results, enabling rigorous hypothesis testing and debugging.
