...
-   **Discrete landscapes**: Space is divided into binary categories
    ...
    unsuitable matrix (value=0). In matrix cells, individuals experience
    maximum mortality ($\mu_{\max} = m \cdot \mu_0$) and, for perfect
    specialists ($\sigma_g = 0$), zero birth rate, making long-term
    persistence effectively impossible. This paradigm corresponds to
    traditional patch-matrix landscape ecology [@Fahrig2003].

Landscapes are generated using `create_fractal_landscape()`, which
produces spatially autocorrelated patterns through iterative neighbor
averaging. At each iteration, every cell value is
updated as a weighted blend of its current value and the mean of its
8-connected neighbors: $v_{t+1} = (1-\beta)\,v_t + \beta\,\bar{v}_t$,
where $\beta$ = `fractality`. Higher `fractality` increases both the
blend weight and the number of iterations, producing broader spatial
autocorrelation. The algorithm does not produce strict fractal
(self-similar) structure; it generates spatially clustered patterns
suitable for ecological simulation.
...
-   **`genotype_means`** (Optimal environmental value): The environmental
    ...
    example, an individual with `genotype_means = 0.3` is genetically adapted
    to environmental value 0.3 (e.g., cool temperatures if the gradient
    represents temperature). When an individual's phenotype
    (genotype_means plus noise from plasticities) exactly matches the local
    environmental value, fitness equals to its maximum.

-   **`genotype_sds`** (Niche width/tolerance): The breadth of
    ...
    tradeoff: values near $\sigma_{min}$ (e.g., `genotype_sds = 0.4`) yield
    ...
    narrow specialist. Larger values (e.g., `genotype_sds = 5.0`) produce
...
-   **`mutation_rates`** (Mutation rate): Controls heritable genetic
    variation transmitted across generations. This parameter determines
    the standard deviation ($\mu_r$) of mutations added to the offspring's
    ...
    composition in response to selection pressures [@Lynch1998].

-   **`plasticities`** (Phenotypic plasticity): Controls non-heritable
    phenotypic variation arising from developmental or environmental
    noise. This parameter determines the standard deviation ($\psi$) of
    ...
    inherited [@Pigliucci2001; @DeWitt2004].

-   **`sampling_points`** (Habitat selection intensity): The number of
    potential dispersal locations an individual evaluates before
    deciding where to move. When `sampling_points = 0`, individuals
    perform a vision-angle-constrained random walk without evaluating
    candidate locations, representing purely passive dispersal. When
    `sampling_points = 1`, a single candidate location is sampled and
    selected without any comparison — functionally passive, but routed
    through the softmax evaluation path internally. As sampling_points
    increases beyond 1, individuals gain the capacity to assess and
    compare multiple locations before
...
**Birth events are asexual:** the offspring is placed at the parent's
exact spatial coordinates and inherits all parent parameters, then
immediately undergoes two sequential steps before entering the
population:

$$g_{\text{offspring}} = g_{\text{parent}} + \varepsilon_{\text{mutation}}, \quad \varepsilon_{\text{mutation}} \sim N(0,\, \mu_r)$$

$$p = g_{\text{offspring}} + \varepsilon_{\text{plasticity}}, \quad \varepsilon_{\text{plasticity}} \sim N(0,\, \psi)$$

where $\mu_r$ = `mutation_rates` and $\psi$ = `plasticities`. Key
distinctions: mutation acts on the genotype (heritable — passed to the
next generation); plasticity acts on the phenotype (non-heritable —
resampled independently at every birth); the phenotype $p$, not the
genotype $g$, determines fitness and habitat selection; and the newborn
occupies the parent's location — dispersal is a separate Gillespie
event. Together these mechanisms enable evolutionary dynamics and
potential local adaptation over multiple generations [@Kawecki2004].
Fitness emerges from the interaction between these traits and local
conditions, as defined in the Fitness section below.
...
-   $p$ (phenotype) is the individual's expressed trait value
    (genotype_means plus developmental noise from the plasticities
...
-   $\sigma$ (genotype_sds) is the individual's tolerance parameter
...
these defaults, all individuals have `genotype_means = 1` (matching habitat
value), `genotype_sds = 0` (zero tolerance, requiring exact environmental
matches), `mutation_rates = 0` (no evolution), `plasticities = 0` (no phenotypic
...
Simulations return a list object containing five components that provide
...
records (events component), reproducibility (parameters component), and
landscape metadata (spatial component):

-   **`summary`**: A list of final population statistics including
    `status` ("surviving" or "extinct"), `final_population_size`
    (number of individuals at simulation end), `total_events` (total
    number of demographic events processed), and `duration` (simulation
    duration in time units). This component provides quick assessment of
    population outcomes without requiring data processing.

-   **`survivors`**: A data frame where each row represents one
    surviving individual at simulation end. Columns include `id`
    (integer individual ID), `genotype` (genotype_means value),
    `width` (niche width, i.e. genotype_sds value), `phenotype`
    (expressed environmental optimum), `x` and `y` (spatial
    coordinates). This component enables analysis of trait
    distributions, spatial patterns, and phenotype-environment matching
    among survivors. When populations go extinct, this component is an
    empty data frame (zero rows).

-   **`events`**: A named list recording every demographic event that
    occurred during the simulation, with each element being a vector of
    equal length (one entry per event). The level of detail depends on
    the `history_detail` top-level argument. At minimum
    (`history_detail="minimal"`), the list contains `times` (when each
    event occurred), `types` (0=death, 1=birth, 2=movement,
    3=emigration, -1=initial state), and `individual_ids`. Standard
    detail (`history_detail="standard"`, the default) additionally
    includes `patch_ids`, `x_coordinates`, `y_coordinates`, and
    `genotypes`. Full detail (`history_detail="full"`) further adds
    `phenotypes` and `widths` (individual niche width at event time). This complete history
    enables analysis of population dynamics, demographic rates over time,
...
-   **`spatial`**: A named list with metadata describing the simulated
    landscape: `world_width` and `world_height` (landscape dimensions in
    world units), `world_size` (the larger of the two dimensions,
    i.e., `max(world_width, world_height)`), and `num_patches` (total
    number of grid cells). These fields support downstream calculations
    such as global density normalization and landscape-scale spatial
    statistics.
...
shared_landscape_params <- list(
  ...
  cell_size                   = 1.0,
  boundary_condition          = 1,     # periodic: individuals wrap around borders (torus topology)
...
omitted: all individuals are genetically identical with `genotype_means = 1.0`
and `genotype_sds = 0`.
...
sim_s1 <- twolife_simulation(
  ...
  # genetic_params omitted → defaults: all genotype_means = 1.0, genotype_sds = 0
...
The simulation results in N = `r pop_s1` (extinction). With
`genotype_sds = 0`, reproduction requires an exact phenotype-environment
match (`h = p`) and is suppressed everywhere else (`b = 0`). Because
the landscape is continuous (range:
`r sprintf("%.2f", min(continuous_landscape))` to
`r sprintf("%.2f", max(continuous_landscape))`), the probability that
genotype_means = 1.0 coincides exactly with any cell's environmental
value is effectively zero. Every individual therefore has `b = 0`
throughout the simulation, while mortality remains positive. The absence
of births, not an excess of mortality over birth rate, drives the
population collapse.

## Scenario 2: Variable Specialists

This scenario adds genetic diversity: each of the 1000 individuals has a
distinct genotype_means spanning 0 to 100, but genotype_sds remains 0
...
individual has `genotype_means = 5.0` and a cell has `environment = 5.0`,
any slight mismatch produces effectively zero fitness when
`genotype_sds = 0`. This demonstrates that genetic diversity alone is
insufficient without tolerance enabling survival across a range of
conditions.

## Scenario 3: Generalists (Niche Width)

This example demonstrates the effect of adding tolerance (genotype_sds).
Each individual has the same genotype_means as Scenario 2 (maintaining
identical genetic diversity), but now with `genotype_sds = 0.4`, enabling
...
not exactly match the local environment. With `genotype_sds = 0.4` (just
...
(`mutation_rates = 0.001`) enables heritable genetic change across
generations, while plasticity (`plasticities = 0.001`) enables non-heritable
