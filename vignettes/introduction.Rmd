...
The rate of decline is controlled by genotype_sd:

-   **Narrow tolerance** (small genotype_sd, e.g., 0.05): Fitness drops
    rapidly with small mismatches. An individual with genotype_sd=0.05
    retains fitness ≈0.6 when mismatch is 0.05, but fitness falls to
    ≈0.14 when mismatch reaches 0.1. These specialists require close
    phenotype-environment matches to survive, performing well only in
    narrow environmental ranges.

-   **Broad tolerance** (large genotype_sd, e.g., 0.3): Fitness declines
    gradually even with substantial mismatches. An individual with
    genotype_sd=0.3 maintains fitness ≈0.95 when mismatch is 0.05, and
    retains fitness ≈0.61 even when mismatch reaches 0.1. These
    generalists survive across wide environmental ranges, though they
    may not achieve the maximum fitness values that specialists attain
...
-   **`plasticity`** (Phenotypic plasticity): Controls non-heritable
    ...
    -   plasticity=0: No phenotypic variation—phenotype equals genotype
        exactly. All individuals with the same genotype_mean express
        identical phenotypes and experience identical fitness
        landscapes.
    -   plasticity=0.1: Moderate variation—phenotype varies around
        genotype with standard deviation 0.1. An individual with
        genotype_mean=0.5 might express phenotype=0.52 or
        phenotype=0.47, sampled once at birth and fixed thereafter.
    -   plasticity=0.2: High variation—phenotype can deviate
        substantially from genotype. An individual with
        genotype_mean=0.5 might express phenotype=0.3 or phenotype=0.7
        (±2σ contains approximately 95% of values).
...
-   **`mutation_rate`** (Mutation rate): Controls heritable genetic
    ...
    -   mutation_rate=0: No evolution—offspring genotype exactly equals
        parent genotype. All descendants retain identical genotype_mean
        values, preventing evolutionary adaptation even under strong
        selection.
    -   mutation_rate=0.05: Moderate evolution—offspring genotype varies
        from parent genotype with standard deviation 0.05. An offspring
        from a parent with genotype_mean=0.5 might have
        genotype_mean=0.52 or 0.48, allowing gradual evolutionary
        change.
    -   mutation_rate=0.1: High evolution—substantial genetic variation
        between parent and offspring. An offspring from a parent with
        genotype_mean=0.5 might have genotype_mean=0.3 or 0.7 (±2σ
        contains approximately 95% of values), facilitating rapid
...
-   **`sampling_points`** (Habitat selection intensity): The number of
    ...
    deciding where to move. This parameter controls habitat selection
    behavior and has direct implications for computational cost:

    -   sampling_points**≤**1: Random dispersal with no habitat
        selection. The individual moves to a randomly chosen location
        without evaluation, representing passive dispersal mechanisms
        such as wind-dispersed seeds or organisms with limited sensory
        capabilities.
    -   sampling_points=10: Moderate selectivity. The individual
        evaluates 10 random locations and selects among them using a
        softmax function weighted by expected fitness at each location.
        This represents organisms with limited but non-zero habitat
        assessment capacity.
    -   sampling_points=100: High selectivity. The individual evaluates
        100 locations, enabling sophisticated habitat selection that
        approaches optimal placement. This represents organisms with
        strong habitat assessment abilities, such as birds evaluating
        multiple potential territories before settling.

    The sampling_points value thus represents assessment capacity rather
    than dispersal distance. Typical values span a biological continuum:
    1 corresponds to passive dispersal, 10-20 to limited assessment,
    50-100 to moderate-to-sophisticated assessment, and 100-200 to
    near-optimal placement. Higher values increase computation time
    linearly—each dispersal with sampling_points=100 evaluates 100
    locations versus 1 location with sampling_points=1. The softmax
    selection process means individuals prefer high-fitness locations
    but retain some randomness, preventing unrealistic determinism where
    all individuals always select the single best available cell.
...
-   **individual_params**: A list containing population initialization
    settings (starting population size, positions, and ages)
...
`max_time` (maximum simulation duration in time units). The
...
next, mutation direction and magnitude, plasticity noise, and habitat
selection decisions when sampling_points\>1—produce identical results
...
-   **`survivors`**: A data frame where each row represents one
    ...
    `phenotype` (current expressed value), `x` and `y` (spatial
    coordinates), `habitat_quality` (local environmental value), and
    `fitness` (current survival probability). This component enables
    analysis of trait distributions, spatial patterns, and
    phenotype-environment matching among survivors. When populations go
    extinct, this component may be NULL or an empty dataframe.

-   **`events`**: A data frame recording every demographic event that
    occurred during the simulation. Each row documents one event with
    columns: `time` (when event occurred), `event_type` ("birth",
    "death", or "dispersal"), `individual_id`, `parent_id` (for births),
    `x` and `y` (location), and relevant trait values. This complete
    history enables analysis of population dynamics, demographic rates
    over time, and lineage tracking.
...
-   **x, y**: Spatial coordinates indicating current location in the
    landscape matrix
-   **habitat_quality**: Environmental value at the individual's current
    location
-   **fitness**: Current survival probability based on
    phenotype-environment match as calculated by the Gaussian fitness
    function
...
autocorrelation), phenotype-environment correlation (strength of
matching), and fitness distribution (population viability indicators,
variance in individual quality).
...
individuals to place at simulation start (here, 100). Additional
optional elements include `initial_positions` (custom starting
locations; default is random placement in habitat cells) and
`initial_ages` (age distribution; default is all age 0). For this
example, we use only the population size and accept default random
placement and ages.

**genetic_params**: A list specifying individual trait values. When
omitted (as in this basic example), all individuals use the default
values described earlier: genotype_mean=1 (matching habitat cells),
genotype_sd=0 (zero tolerance, requiring exact matches), mutation_rate=0
(no evolution), plasticity=0 (no noise), sampling_points=0 (random
dispersal). These defaults create a genetically uniform population with
zero tolerance and random dispersal—appropriate for testing landscape
effects in binary habitat-matrix simulations without genetic
complications. However, as we will see in Section 6, these same defaults
produce extinction in continuous landscapes.
...
population dynamics without excessive computation. Alternatively,
`max_time` could specify simulation duration in time units rather than
number of events. The choice between max_events and max_time depends on
research questions: max_events ensures comparable demographic histories
across simulations, while max_time ensures comparable temporal scales.

**master_seed**: An integer that controls all random processes (initial
placement when positions aren't specified, which individual
reproduces/dies/disperses next, mutation direction and magnitude when
mutation_rate\>0, plasticity noise when plasticity\>0, and habitat
selection decisions when sampling_points\>1). Using identical
master_seed with identical parameters produces exactly identical
results—every demographic event occurs in the same sequence at the same
time. Using different seeds with identical parameters produces
independent realizations—the demographic processes (birth rates, death
rates, dispersal patterns) are identical but specific outcomes differ
due to stochasticity. This is analogous to experimental trials: using
the same seed is like recording a single trial repeatedly, while using
different seeds simulates multiple independent trials under the same
experimental conditions. This distinction enables quantifying outcome
variability under fixed conditions.
...
occurs because zero tolerance means individuals require exact
environmental matches (value=1.0) that don't exist in the continuous
landscape (range: `r sprintf("%.2f", min(continuous_landscape))` to
`r sprintf("%.2f", max(continuous_landscape))`). Even if cells near 1.0
exist, the Gaussian fitness function with genotype_sd=0 produces
effectively zero fitness for any mismatch, no matter how small. This
demonstrates that genetic uniformity combined with specialist strategies
is incompatible with persistence in heterogeneous continuous
environments.
...
-   **Simulation duration**: Cost scales linearly with `max_events` or
    `max_time`.
